...
//referencia 
Surf::Surf(IplImage *img, KVector &kps): kps(kps)
{
  this->img = img;
...
void Surf::getOrientation()
{
  ...
  const int s = fRound(scale), r = fRound(kp->y), c = fRound(kp->x);
  ...
  for(int i = -6; i <= 6; ++i) 
  {
    for(int j = -6; j <= 6; ++j) 
    {
      if(i*i + j*j < 36) 
      {
        ...
        Ang[idx] = getAng(resX[idx], resY[idx]);
        ++idx;
      }
    }
  }
  ...
  for(ang1 = 0; ang1 < 2*pi;  ang1+=0.15f) {
    ...
    if (sumX*sumX + sumY*sumY > max) 
    {
      ...
      orientation = getAng(sumX, sumY);
    }
  }
  kp->orientacion = orientation;
}
...
float Surf::gaussian(int x, int y, float sig)
{
  return (1.0f/(2.0f*pi*sig*sig)) * exp( -(x*x+y*y)/(2.0f*sig*sig));
}

float Surf::gaussian(float x, float y, float sig)
{
  return 1.0f/(2.0f*pi*sig*sig) * exp( -(x*x+y*y)/(2.0f*sig*sig));
}
...
float Surf::getAng(float X, float Y)
{
  if(X > 0 && Y >= 0)
    return atan(Y/X);
